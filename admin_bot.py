...
def get_model_avg(message):
    ...
    #bot.send_message(message.chat.id, name)
    bot.send_message(message.chat.id, 'Введите цену: ')
    bot.register_next_step_handler(message, get_price_avg)

def get_price_avg(message):
    user_id = message.chat.id
    notes[user_id] += message.text + ';'
    #bot.send_message(message.chat.id, name)
    print(notes[user_id])
    ino = cl.add_avg(notes[user_id])
    notes[user_id] = ''
    s.sendto(ino.encode('utf-8'), server)
    data, addr = s.recvfrom(4294967296)
    data = data.decode('utf-8')
...
def get_id_market(message):
    user_id = message.chat.id
    notes[user_id] = message.text
    ino = cl.delete_market(notes[user_id])
    notes[user_id] = ''
    s.sendto(ino.encode('utf-8'), server)
    data, addr = s.recvfrom(4294967296)
    data = data.decode('utf-8')
...
def get_del_avg(message):
    user_id = message.chat.id
    notes[user_id] = message.text
    ino = cl.delete_avg(notes[user_id])
    notes[user_id] = ''
    s.sendto(ino.encode('utf-8'), server)
    data, addr = s.recvfrom(4294967296)
    data = data.decode('utf-8')
